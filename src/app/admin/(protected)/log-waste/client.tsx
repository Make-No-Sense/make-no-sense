    ...
    <div className="p-4 md:p-10 flex flex-col gap-8">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-black/5 p-4 sm:p-6 max-w-xl mx-auto w-full">
        ...
        <form ref={formRef} onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="font-display text-xs uppercase tracking-wide text-[#1B3A5C]">Reason</label>
      ...
      <div>
        <p className="font-display text-xs uppercase tracking-widest text-[#1B3A5C]/50 mb-4">Recent Waste</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
            <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-black/5">
              <div>
                <h2 className="font-display text-lg uppercase tracking-wide text-[#1B3A5C]">Edit Waste Log</h2>
            ...
            <form onSubmit={handleEditSubmit} className="px-6 py-5 flex flex-col gap-4">
              <div className="flex flex-col gap-1.5">
                <label className="font-display text-xs uppercase tracking-wide text-[#1B3A5C]">
                  Quantity Wasted ({editingLog.inventory_items.unit})
                </label>
              ...
              <div className="flex flex-col gap-1.5">
                <label className="font-display text-xs uppercase tracking-wide text-[#1B3A5C]">Reason</label>
                <select name="reason" required defaultValue={editingLog.reason}
                  className="border border-black/10 rounded-lg px-3 py-2 text-sm text-[#1B3A5C] focus:outline-none focus:ring-2 focus:ring-[#B83232]/40 bg-white">
                  {REASONS.map((r) => (
                    <option key={r} value={r}>{r.charAt(0).toUpperCase() + r.slice(1)}</option>
                  ))}
